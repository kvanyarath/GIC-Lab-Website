...
<script>
    ...
    @if(!empty($projects) && isset($projects[0]))
        var projectID1 = document.getElementById({{$projects[0]->id}});
    ...
    @if(!empty($projects) && isset($projects[1]))
        var projectID2 = document.getElementById({{$projects[1]->id}});
    ...
    function fetchRecords(url){
        ...
        success: function(response){
            var textParaph;
            textParaph = "<h4 class=\"h1 g-font-size-40--md g-mb-40 text-center\">"+response['data'].developer_name+"</h4>" + response['data'].html_text;
            $('#modalParagrah').append(textParaph);
        }
        });
    }
    ...
    function fetchProjectRecords(projectID){
        ...
        success: function(response){
            ...
            if (response['data'].ongoing_project==1){
